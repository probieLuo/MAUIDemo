...
{
    internal class PersistentCacheService_ClanRank
    {
        private string CacheFilePath = "cache_ClanRank.json";
        private string CacheKey = "";
        private string LocationId = "";
        public PersistentCacheService_ClanRank(string locationId)
        {
            LocationId = locationId;
            // 设置缓存文件路径
            CacheFilePath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "cache");
            CacheFilePath = Path.Combine(CacheFilePath, $"cache_ClanRank_{locationId}.json");
        ...
        // 获取缓存中的集合
        public async Task<ClanRankingList> GetCollection()
        {
            // 检查缓存文件是否存在
            if (File.Exists(CacheFilePath))
            {
                ...
                var cache = JsonConvert.DeserializeObject<CacheData_ClanRank>(cacheData);
                ...
                else
                {
                    ...
                    var cache1 = JsonConvert.DeserializeObject<CacheData_ClanRank>(cacheData);
        ...
        // 更新缓存中的集合
        private async Task UpdateCollection(ClanRankingList newCollection)
        {
            // 设置缓存的过期时间为1天
            var expiration = DateTime.Now;

            // 创建缓存数据对象
            var cacheData = new CacheData_ClanRank
            {
        ...
        // 从数据源获取数据
        private async Task<ClanRankingList> GetNewDataFromDataSource()
        {
            ...
            return new ClanRankingList();
        }
    }

    // 缓存数据类
    public class CacheData_ClanRank
    {
        public ClanRankingList Collection { get; set; }
